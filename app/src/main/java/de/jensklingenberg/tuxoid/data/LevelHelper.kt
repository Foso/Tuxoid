...
class LevelHelper(private val gameState: GameState, private val elementDataSource: ElementDataSource) : Timer_Water.TimerClock, Timer_ice.TimerClock,
    ...
    fun move_To_Collectable(y: Int, x: Int, aktObject: Int, callingCharacter: Element) {
        when (aktObject) {
            ElementType.FISH -> {
                setNewElement(callingCharacter.typeId, Coordinate(gameState.aktEbene, y, x))
     ...
     *
     */
    fun setNewElement(newType: Int, newCoord: Coordinate) {
        ...
        when (oldType) {
            ElementType.SWITCH -> {
                if (ElementType.DIAMOND_CRATE == newType) {
                    setNewElement(ElementType.SWITCH_CRATE_BLOCK, Coordinate(gameState.aktEbene, y, x))
    ...
    fun checkMove(
        ...
        if (ElementGroup.charNPC == firstElement.elementGroup) {

            val npc = (firstElement as NPC).npcNumber
        ...
        when (getElementAt(prevCoord).elementGroup) {
            ...
            ElementGroup.charNPC, ElementGroup.charPlayer -> {
                move_character(
                        direction, z, y, x, firstElement, dirX, dirY, aktObjectTypeId,
                        nextObjectGroup
                )
            }


            ElementGroup.TeleportOut -> {
    ...
    private fun move_moveable(
            ...
            dirY: Int, aktObject: Int, firstElement: Element
    ) {
        ...
        when (aktObject) {
            ...
            ElementType.CROSS_CRATE -> {
                ...
                if (ele.moveRule.canMove(nexDes)) {
                    checkMove(direction, aktObject, Coordinate(z, y + dirY, x + dirX), firstElement)
                }
            }
    ...
    fun move_to_destination(
            ...
            dirX: Int, dirY: Int, aktObject: Int, callingCharacter: Element
    ) {
        val aktElement = ElementFactory.elementFactory(aktObject)
        ...
        when (aktObject) {
            ...
            ElementType.LADDER_UP -> if (ElementType.PLAYER == Object) {
                gameState.aktEbene--
                setNewElement(ElementType.PLAYER, Coordinate(gameState.aktEbene, y, x))
                refreshListener?.onRefresh(gameState.levelData!![gameState.aktEbene])
            }
    ...
    fun move_to_arrow(
            ...
            aktObject: Int, callingCharacter: Element
    ) {
